...
numjobs=8
builddir=build
outdir=/home/alvaro/deconvolutiontest/richardsonlucy/saturno_extra_noise
...
options="--kernel=tess-psf1.fits -A richardson-lucy --khdu=0 --hdu=0 -a 0.4"
...
values=(1 5 10 20 40 60 80 100 120 140 160 180 200 220 240)
    ...
    for L in "${values[@]}"; do
    "$utility" $arguments $options -i $L -o alpha04/${L}_iteration.fits $extraopts
done
    # Clean up.
    rm -rf .gnuastro
fi
